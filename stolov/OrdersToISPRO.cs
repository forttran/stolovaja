	...
	internal class OrdersToISPRO {
		...
		public static string Reserved_Rcd(string TableName, string ColumnName) {
			try {
				...
				currentClassLogger.Error("OrdersToISPRO.Reserved_Rcd.Ошибка генератора Rcd");
				currentClassLogger.Error("Ошибка генератора Rcd: " + excp.Message + "; Source: " + excp.Source); return "-1"; 
			}
		}
		//Функция получения RcdNmr
		public string GetRcdNmr() {
			string sql = "";
			try {
				string date = DateTime.Today.ToString("yyyyMMdd");
				sql = "SELECT ISNULL(MAX(PrdZkg_RcdNmr),0)  FROM PRDZKG WHERE PrdZkg_RcdNmr < 9999999 and (PRDZKG_Dt = CONVERT(date, '" + date + "') ) ";
				DB.getInstance();
				...
				currentClassLogger.Error("OrdersToISPRO.Reserved_Rcd.Ошибка получения RcdNmr");
		...
		public void InsertOrder(string PrdZkg_Rcd, string RcdNmr, string date, double sum, double nalog, double usr, int tab) {
			try {
				...
				try {
					...
					currentClassLogger.Error(sql);
					currentClassLogger.Error("OrdersToISPRO.insertOrder.Ошибка вставки в таблицу PrdZkg");
					currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
				};
				if (tab != -1) {
					...
					try {
						...
						currentClassLogger.Error(sql);
						currentClassLogger.Error("OrdersToISPRO.insertOrder.Ошибка вставки в таблицу UFPRV");
						currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
					};
				}
				...
				MessageBox.Show("Ошибка обработки вставки заказа");
				currentClassLogger.Error("OrdersToISPRO.insertOrder.Ошибка обработки вставки заказа");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			};

		}
		//Вставка позиции в заказе в базу данных
		public void InsertPosition(string trds_rcd, string zkg_rcd, string date, string RcdOra, string RcdNom, string RcdPrc, string EiQt, string cnt, string sum, string tax) {
			try {
				string sql = "declare @dt VARCHAR(8) "
						+ "set @dt = '" + date + "' "
						+ "DECLARE @cnt FLOAT "
						+ "DECLARE @sum FLOAT "
						+ "DECLARE @tax FLOAT "
				...
				try {
					...
					currentClassLogger.Error(sql);
					currentClassLogger.Error("OrdersToISPRO.insertPosition.Ошибка вставки в таблицу TRDS");
					currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
				};
				...
				try {
					...
					currentClassLogger.Error(sql);
					currentClassLogger.Error("OrdersToISPRO.insertPosition.Ошибка вставки в таблицу ATrdsTax");
					currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
				};
			} catch (Exception e) {
				MessageBox.Show("Ошибка обработки вставки позиции в заказ");
				currentClassLogger.Error("OrdersToISPRO.insertPosition.Ошибка обработки вставки позиции в заказ");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			};
		...
		public void GenerateFile(string RcdNmr, OrderElement elements) {
			...
			try {
				...
				using StreamWriter streamWriter = new((Stream)fileStream, Encoding.UTF8);
				DataRow[] result = elements.order.Select();
				foreach (DataRow row in result) {
					//Боремся с проклятой проблемой разделителей точка или запятая
					string tmp = "";
					try {
						tmp = Convert.ToDouble(row[6]).ToString("0.##").Replace(",", "."); ;
					} catch {
						tmp = Convert.ToDouble(row[6].ToString().Replace(".", ",")).ToString("0.##").Replace(",", ".");
					}
					string str12 = row[2].ToString().Remove(0, 2) + ";1;" + row[5].ToString().Replace(",", ".") + ";" + tmp + ";" + row[4].ToString() + ";";
					str12 += "1;";//секция
				...
				currentClassLogger.Error("OrdersToISPRO.generateFile.Ошибка записи первого файла");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			}
			...
			try {
				...
				string str12;
				if (elements.user == null || elements.ret == true) {
					str12 = "0;" + elements.GetSum().ToString("0.##").Replace(",", "."); ;
				} else {
					str12 = "3;" + elements.GetSum().ToString("0.##").Replace(",", "."); ;
				}
				streamWriter.WriteLine(str12);
			} catch (Exception e) {
				MessageBox.Show("Ошибка записи второго файла");
				currentClassLogger.Error("OrdersToISPRO.generateFile.Ошибка записи второго файла");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			}
		}
		//Главная функция вставки в ИС-ПРО
		public void TMPtoPrdZkg() {
			try {
				...
				foreach (OrderElement item in data.OrderElementList.ToArray()) {
					try {
						...
						foreach (DataRow row in result) {
							...
							string tax = "";
							try {
								tax = (Convert.ToDouble(row[6]) * 0.13).ToString();
							} catch {
								tax = (Convert.ToDouble(row[6].ToString().Replace(".", ",")) * 0.13).ToString();
							}
							currentClassLogger.Debug("|" + row[0] + "|" + row[1] + "|" + row[2] + "|" + row[3] + "|" + row[4] + "|" + row[5] + "|" + row[6] + "|" + row[7] + "|" + row[8] + "|" + row[9]);
							InsertPosition(TrdS_Rcd, PrdZkg_Rcd, today.ToString("yyyyMMdd"), row[7].ToString(), row[0].ToString(), row[8].ToString(), row[9].ToString(), row[5].ToString(), row[6].ToString(), tax);
						}
						GenerateFile(RcdNmr, item);
					} catch (Exception e) {
						MessageBox.Show("Ошибка итерации");
						currentClassLogger.Error("OrdersToISPRO.TMPtoPrdZkg.Ошибка итерации");
						currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
					}
				...
				MessageBox.Show("Ошибка главного модуля загрузки ИС-ПРО");
				currentClassLogger.Error("OrdersToISPRO.TMPtoPrdZkg.Ошибка главного модуля загрузки ИС-ПРО");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			}
		}
		...
		public void Prdzkgt(string date) {
			...
			try {
				...
				currentClassLogger.Error(sql);
				currentClassLogger.Error("OrdersToISPRO.prdzkgt.Ошибка работы с таблицей PRDZKDT");
				currentClassLogger.Error("Ошибка: " + e.Message + "; Source: " + e.Source);
			};
		}
	}
